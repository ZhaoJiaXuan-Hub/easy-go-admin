<template>
  <div class="gi_page menu-manage">
    <a-card>
      ...
      <a-table ref="tableRef" row-key="id"  :data="menuList" :loading="loading" :bordered="{
          wrapper: true,
      }"
          ...
          <a-table-column title="操作" :width="200" align="left" :fixed="!isMobile() ? 'right' : undefined">
            <template #cell="{ record }">
              <a-space>
                ...
                <a-link v-if="[1, 2].includes(record.type)" @click="onAddById(record)">
                  新增
                </a-link>
                <a-popconfirm type="warning" content="您确定要删除该项吗?" @click="onDelete(record)">
...
const onAddById = (item: MenuItem) => {
  AddMenuModalRef.value?.add(item.id)
}
